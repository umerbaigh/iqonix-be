...
module.exports = createCoreController(
  ...
  ({ strapi }) => ({
    async GetDepartmentProducts(ctx) {
      try {
        const { slug } = ctx.params; // Department slug
        const {
          sales = false,
          delivery,
          furniture_color,
        ...
        if (!slug) {
          return ctx.badRequest("Department slug is required.");
        }
        ...
        // Fetch the department using the slug and populate its products
        const department = await strapi.entityService.findMany(
          "api::department.department",
          {
        ...
        let allProducts = department[0].products || [];
        ...
        ctx.throw(500, "Failed to fetch department products.");
      }
    },
  })
);
