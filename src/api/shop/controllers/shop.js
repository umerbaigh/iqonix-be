...
module.exports = createCoreController("api::shop.shop", ({ strapi }) => ({
  async GetShopProducts(ctx) {
    try {
      const { slug } = ctx.params; // Shop slug
      const {
        sales = false,
        delivery,
        furniture_color,
      ...
      if (!slug) {
        return ctx.badRequest("Shop slug is required.");
      }
      ...
      // Fetch the shop using the slug and populate its products
      ...
      let allProducts = shop[0].products || [];
      ...
      ctx.throw(500, "Failed to fetch shop products.");
    }
  },
}));
